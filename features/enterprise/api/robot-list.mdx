...
| **Url** | [https://api-rpa.bazhuayu.com/api/v1/bots/management/bots?search=](https://api-rpa.bazhuayu.com/api/v1/bots/management/bots?search=)\{search\}&start=\{start\}&take=\{take\} |
| --- | --- |
| **请求方式** | **GET** |
...
| **参数名称** | **类型** | **必须** | **描述** |
| --- | --- | --- | --- |
| items | **参数名称** **类型** **必须** **描述** id string 是 机器人 Id name string 是 机器人名称 intro string   机器人描述 machineName string 是 机器人机器名 machineIp string 是 机器人机器 IP version string 是 机器人客户端版本 isConnected bool 是 机器人是否连接上服务 isEnabled bool 是 机器人是否可用 executionStatus string 是 机器人状态： Empty - 空 Waiting - 待执行 Scheduled - 任务已分配 Delivered - 机器人已接收任务 Executing - 执行中 Stopping - 停止中 Stopped - 已停止 Finished - 已完成 Failed - 执行失败 Cancelled - 已取消 lastConnectingTime string 是 最后一次连接服务时间 flowId string   正在运行的应用 Id flowProcessNo string   正在运行的应用运行批次 | 是 | 机器人列表 |
| **参数名称** | **类型** | **必须** | **描述** |
| id | string | 是 | 机器人 Id |
| name | string | 是 | 机器人名称 |
| intro | string |  | 机器人描述 |
